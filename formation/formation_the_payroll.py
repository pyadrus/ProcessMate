...
def formation_the_payroll():
    ...
    send_keys('{ENTER}', with_spaces=True,
              pause=1)  # Если ENTER 2, то как будто срабатывает 2 нажатие если ENTER работает как одно
    send_keys('{PGUP}', with_spaces=True, pause=1)
    for _ in range(13):
        send_keys('{INSERT}', with_spaces=True, pause=0.1)
    send_keys('{ENTER}',
              with_spaces=True)  # Если ENTER 2, то как будто срабатывает 2 нажатие если ENTER работает как одно
